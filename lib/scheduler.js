 ...
 * @param {Function} [options.verifyArtifact] - async (post) => verifyGitArtifact-shaped
 *   result. When provided, each due campaign post is checked BEFORE calling
 *   gitPublish — if a verified, exact-match artifact already exists (e.g.
 *   from a prior run where the commit succeeded but the KV write recording
 *   'published' failed and rolled back to 'scheduled' — the 2026-07-28
 *   duplicate-commit gap), the post is reconciled straight to 'published'
 *   without ever calling gitPublish, so a second commit is never created.
 ...
 * @returns {Promise<{ processed: number, published: string[], reconciled: string[], failed: { id: string, error: string }[] }>}
 */
export async function processScheduledPosts(store, options = {}) {
  ...
  for (const post of scheduledPosts) {
    try {
      ...
      // Reconcile-before-publish: scoped to campaign posts only (this
      // project's only automated-publish path with a well-defined expected
      // artifact shape — never arbitrary/legacy posts). A read-only check;
      // skips the real Git commit entirely when one already genuinely
      // exists, closing the 2026-07-28 duplicate-commit gap.
      const isCampaignPost = typeof post.id === 'string' && post.id.startsWith('campaign-');
      let verified = null;
      if (isCampaignPost && typeof options.verifyArtifact === 'function') {
        try {
          verified = await options.verifyArtifact(publishing);
        } catch {
          verified = null;
        }
      }
      const alreadyVerified = !!(verified && verified.verified);

      if (requireGitPublish && !alreadyVerified && typeof options.gitPublish !== 'function') {
      ...
      if (alreadyVerified) {
        gitResult = {
          ok: true,
          commit: verified.branchCommitSha,
      ...
      const publishedPost = await transitionPost(store, post.id, 'published', {
        actor: 'scheduler',
        meta: { git: gitResult, reconciled: alreadyVerified },
      });
      ...
      try {
        await record(store, {
          actor: 'scheduler',
          action: alreadyVerified ? 'post.reconciled_published' : 'post.auto_publish',
          target_type: 'post',
          target_id: post.id,
          meta: {
      ...
      if (alreadyVerified) reconciled.push(post.id);
    } catch (e) {
  ...
  return { processed: scheduledPosts.length, published, reconciled, failed };
}
      ...
      if (verified && verified.verified) {
        ...
        try {
          await record(store, {
            ...
            meta: {
              commit: verified.branchCommitSha,
              pageBlobSha: verified.pageBlobSha,
              sitemapBlobSha: verified.sitemapBlobSha,
              repository: verified.repository,
              branch: verified.branch,
        ...
        await transitionPost(store, post.id, 'scheduled', {
          actor: 'scheduler',
          meta: { reconciled: true, reverted_stale_publishing: true, verify_error: verified?.mismatchReason || verified?.error || 'no_artifact_found' },
        });
        try {
          await record(store, {
            actor: 'scheduler',
            ...
            target_type: 'post',
            target_id: post.id,
            meta: { reason: verified?.mismatchReason || verified?.error || 'no_artifact_found' },
          });
        } catch { /* best effort */ }
