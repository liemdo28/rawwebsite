...
export async function commitToGit(env, post, options = {}) {
  ...
  try {
    ...
    await updateContentIndex(token, owner, repo, branch, post);

    // Render and commit the actual routable static page.
    const pagePath = `public/${post.slug}.html`;
    const pageHtml = renderArticlePage(post);
    const pageSha = await getFileSha(token, owner, repo, branch, pagePath);
    await createOrUpdateFile(token, owner, repo, branch, {
      path: pagePath,
      content: pageHtml,
      message: `Publish page: ${post.slug}.html`,
      sha: pageSha,
      committer: { name: 'RawWebsite Bot', email: 'bot@rawsushibar.com' },
    });

    // Add the new page to sitemap.xml.
    await updateSitemap(token, owner, repo, branch, post);

    return {
      ok: true,
      commit: commitResult.commit?.sha || commitResult.content?.sha,
      files: [filePath, 'content/index.json', pagePath, 'public/sitemap.xml'],
...
/**
 * Add the post's URL to public/sitemap.xml (idempotent — no-op if present).
 */
async function updateSitemap(token, owner, repo, branch, post) {
  const filePath = 'public/sitemap.xml';
  const existingSha = await getFileSha(token, owner, repo, branch, filePath);
  let current = '<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n</urlset>\n';
  if (existingSha) {
    ...
    if (res.ok) {
      ...
      if (data && typeof data.content === 'string') {
        ...
        } catch { /* fall back to the default empty sitemap below */ }
      }
    }
  }
  const updated = addUrlToSitemap(current, post);
  if (updated === current) return; // URL already present, nothing to do
  await createOrUpdateFile(token, owner, repo, branch, {
    path: filePath,
    content: updated,
    message: `Add ${post.slug} to sitemap`,
    sha: existingSha,
    committer: { name: 'RawWebsite Bot', email: 'bot@rawsushibar.com' },
  });
}
