...
export async function commitToGit(env, post, options = {}) {
  ...
  try {
    ...
    const currentRootSitemap = await readTextFile(token, owner, repo, branch, 'sitemap.xml',
      fallbackSitemap);
    ...
    const rootSitemapXml = addUrlToSitemap(currentRootSitemap, post);
    ...
    const files = [
      ...
      { path: rootPagePath, content: pageHtml },
      { path: 'sitemap.xml', content: rootSitemapXml },
