 ...
 * used by lib/scheduler.js's stale-'publishing' reconciliation (see the
 * 2026-07-28 KV-quota incident's critical partial-success case: a Git commit
 * can succeed and then the KV write that would record 'published' can fail,
 * leaving the post stuck in 'publishing' with no local proof of the commit).
 *
 * Reads GitHub's Contents API directly — never the live site — so a
 * Cloudflare Pages SPA-fallback false-200 (see the 2026-07-28 soft-404
 * incident this same project hit) can never be mistaken for artifact proof.
 * Read-only: never creates a commit, so reconciliation can never duplicate
...
export async function verifyGitArtifact(env, post) {
  ...
  try {
    const publicPagePath = `public/${post.slug}.html`;
    const pageContent = await readTextFile(token, owner, repo, branch, publicPagePath, null);
    if (pageContent === null) {
      return { ok: false, error: 'artifact_missing_page' };
    }

    const sitemapXml = await readTextFile(token, owner, repo, branch, 'public/sitemap.xml', '');
    const siteUrl = `https://www.rawsushibar.com/${post.slug}.html`;
    if (!sitemapXml.includes(`<loc>${siteUrl}</loc>`)) {
      return { ok: false, error: 'artifact_missing_sitemap_entry' };
    }
