...
test('processScheduledPosts: calls gitPublish callback', async () => {
  ...
  const gitPublish = async (p) => {
    ...
    gitPublishPost = p;
    return { ok: true, commit: 'abc123', repository: 'acme/website', branch: 'main', files: ['content/posts/git-callback-test.md', 'content/index.json', 'git-callback-test.html', 'sitemap.xml', 'public/git-callback-test.html', 'public/sitemap.xml', 'functions/_validPaths.mjs'] };
  };
...
test('processScheduledPosts: successful commit publishes with git metadata', async () => {
  ...
  const result = await processScheduledPosts(store, {
    gitPublish: async () => ({
      ...
      branch: 'main',
      files: ['content/posts/successful-commit.md', 'content/index.json', 'successful-commit.html', 'sitemap.xml', 'public/successful-commit.html', 'public/sitemap.xml', 'functions/_validPaths.mjs'],
    }),
  });
...
test('processScheduledPosts: retry after partial failure can publish later', async () => {
  ...
  const second = await processScheduledPosts(store, {
    gitPublish: async () => ({
      ...
      repository: 'acme/site',
      branch: 'main',
      files: ['content/posts/partial-retry.md', 'content/index.json', 'partial-retry.html', 'sitemap.xml', 'public/partial-retry.html', 'public/sitemap.xml', 'functions/_validPaths.mjs'],
    }),
  });
...
test('processScheduledPosts: already-published artifact idempotent retry can publish once', async () => {
  ...
  const result = await processScheduledPosts(store, {
    gitPublish: async () => ({
      ...
      action: 'noop',
      idempotent: true,
      files: ['content/posts/idempotent-retry.md', 'content/index.json', 'idempotent-retry.html', 'sitemap.xml', 'public/idempotent-retry.html', 'public/sitemap.xml', 'functions/_validPaths.mjs'],
    }),
  });
